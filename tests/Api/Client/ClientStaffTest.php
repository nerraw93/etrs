...
class ClientStaffTest extends TestCase
{
     ...
     * @test
     */
    public function canGetClientStaffDetails()
    {
        $clientStaff = $this->findRandomData('client_staffs');

        $client = User::find($clientStaff->client_id);
        $this->asClient($client->email);
        $response = $this->json('GET', route('api.client.staff.details', ['id' => $clientStaff->id]));
        $data = $response->getData();
        $response
            ->assertStatus(self::RESPONSE_SUCCESS)
            ->assertJson([
        ...
        $this->assertEquals($staff->client_id, $this->user->id);
        $this->assertObjectHasAttribute('user', $staff);
        $this->assertEquals($staff->staff_id, $staff->user->id);
    }
