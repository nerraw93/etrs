...
class ClientPatientTest extends TestCase
{
    ...
    {
        $patient = Patient::orderByRaw('RAND()')->with('client')->first();
        ...
        $databaseCount = Patient::where('client_id', $this->user->id)->count();
        $this->assertEquals($databaseCount, $data->patients->total);
    }
    ...
    public function canStoreClientPatient()
    {
        ...
        $this->assertObjectHasAttribute('patient', $data);
        $this->assertObjectHasAttribute('id', $data->patient);
    }

    /**
     * @test
     */
    public function clientCanStorePatientIfCustomIdIsNull()
    {
        ...
        $this->assertNull($patient->client_custom_id);
    }
     ...
     * @test
     */
    public function clientCanGetPatientDetails()
    {
        $patient = Patient::orderByRaw('RAND()')->with('client')->first();

        $this->asClient($patient->client->email);
        $response = $this->json('GET', route('api.client.patient.details', ['id' => $patient->id]));
        $data = $response->getData();
        $response
            ->assertStatus(self::RESPONSE_SUCCESS)
            ->assertJson([
        ...
        $this->assertObjectHasAttribute('patient', $data);
        $responsePatient = $data->patient;

        $this->assertEquals($responsePatient->id, $patient->id);
        $this->assertEquals($responsePatient->client_id, $this->user->id);
    }
}
