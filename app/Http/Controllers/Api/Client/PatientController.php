...
class PatientController extends Controller
{
    ...
    public function archive(Request $request, $id)
    {
        ...
        if ($patient->client_id != get_client_id()) {
            return errorify(trans('message.client.patient.error.patient_not_owned'));
        }

        $patient->delete();
        return successful(trans('message.client.patient.success.archived'));
    }
    ...
    {
        $patient = Patient::owned()->findOrFail($id);
        return success_data(compact('patient'));
    }
}
