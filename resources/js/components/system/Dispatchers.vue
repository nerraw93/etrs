<template>
    <section class="dispatcher">
        <div class="column portlet">
            ...
            <p v-if="dispatchers.length === 0" class="text-center pt-5 pr-5 pl-5 pb-5">No data available.</p>
            <b-table v-else :data="dispatchers" bordered striped hoverable>
            ...
            <div class="pagination-controls mt-2">
                ...
                <b-button
                    type="is-danger"
                    :disabled="page === dispatcherObj.last_page"
                    @click="next()"
                >
                    Next
                </b-button>
...
export default {
    ...
    computed: {
        ...
        dispatcherObj()
        {
            return this.$store.getters['dispatcher/data'];
        },
    },
    ...
    methods: {
         ...
         */
        add() {
            this.$store.dispatch('dispatcher/store', {data: this.storeData})
                .then((data) => {
         ...
         */
        submitEdit() {
            this.$store.dispatch('dispatcher/update', {data: this.editData})
                .then((data) => {
                    this.successToast(data.message)
        ...
        archive(dispatcher) {
            Dialog.confirm({
                title: 'Deleting dispatcher',
                message: 'Are you sure you want to <b>delete</b>? This action cannot be undone.',
                confirmText: 'Confirm',
                type: 'is-danger',
                hasIcon: true,
                onConfirm: () => this.delete(dispatcher)
            })
        },

        delete(dispatcher)
        {
            this.$store.dispatch('dispatcher/delete', {data: dispatcher})
                .then((data) => {
                    this.successToast(data.message)
        ...
        fetch()
        {
            ...
            this.$store.dispatch('dispatcher/fetch', {payload: payload})
                .then(({data}) => {
                })
                .catch((err) => {
...
@import "../../../sass/components/system/dispatchers.scss";
</style>
