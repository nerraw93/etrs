      ...
      <form @submit.prevent="submit">
        <div class="column">
          ...
          <div class="columns">
            <div class="column is-half">
              <ul>
                <li v-for="(item, index) in selectedSources" :key="index">
                  {{ item.code + ' - ' + item.name }} 
                  <b-icon
                    icon="close"
                    class="app-text-primary pointer"
                    size="is-small"
                    @click.native="removeSelectedSource(index)"
                  />
                </li>
              </ul>
            </div>
      ...
      try {
        ...
        if (data.success) {
          ...
          this.selectedSources.forEach((source) => {
            http.postJSON(`/api/admin/client/${data.client.id}/sources/store`, {
                user_id:  data.client.code,
                source_id: source.id
            })
                .then(({data}) => {
