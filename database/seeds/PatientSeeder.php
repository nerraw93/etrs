...
class PatientSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        for ($i=0; $i < 15; $i++) {
            ...
            $patient->occupation = substr($faker->jobTitle, 0, 40);
            $patient->save();
        }

        // Seed on perfect_client
        $perfect = User::where('email', 'perfect_client@gmail.com')->first();
        $id = global_prefix() . $count . \Carbon\Carbon::now()->timestamp;

        $patient = new Patient();
        $patient->client_id = $perfect->id;
